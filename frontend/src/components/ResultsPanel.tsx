    ...
    <aside className="performance-panel" data-risk={domain}>
      ...
      {tab === "performance" && <div className="result-pane active">
        <section className="cd-hero"><div className="cd-value"><span>Estimated Cd</span><strong>{prediction?.cd.toFixed(4) ?? "—"}</strong></div>
          <div className={`delta-badge ${delta == null || Math.abs(delta) < .00005 ? "neutral" : delta < 0 ? "better" : "worse"}`}>{delta == null ? "Waiting for baseline" : Math.abs(delta) < .00005 ? "This is the baseline concept" : `${delta < 0 ? "↓" : "↑"} ${Math.abs(deltaPercent || 0).toFixed(1)}% vs baseline · ${delta > 0 ? "+" : ""}${delta.toFixed(4)}`}</div>
