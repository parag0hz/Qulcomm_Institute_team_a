...
|---|---|---:|---:|---:|
| **PointNet** | Point cloud (2048 pts) | **0.853 ± 0.031** | **6.86** | **88.5 %** |
...
The gap is widest exactly where it matters. On **Estate** bodies, parameter models collapse — AutoGluon reaches R² +0.038 and RandomForest goes *negative* (−0.257), i.e. worse than predicting the mean. PointNet holds at **+0.786**. Global averages hide this, which is why every number here is decomposed by body type.
...
**Why PointNet reads 0.968 in one table and 0.853 in the other.** These measure different things, and neither is cherry-picked. The benchmark number uses the official split over all 7,713 designs. The comparison number uses only the 3,704 designs that have *both* a point cloud and a parameter row — the intersection needed to put both tracks on identical footing. That halves the training data and narrows Cd variance from 0.037 to 0.023, which mechanically depresses R². AutoGluon actually scores slightly *higher* under the stricter protocol (0.549 → 0.573). The **+0.28 gap between shape and parameters is the real figure**, because it is the only one measured under matched conditions.
...
- Absolute Cd certification is out of scope. CFA ranks and screens; a wind tunnel or high-fidelity CFD decides.
...
```bash
docker build -t cfa . && docker run -p 8000:8000 cfa
```
